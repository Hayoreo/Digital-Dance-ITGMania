...
if mods.ShowFaPlusWindow then
	...
	Colors = {
		...
		SL.JudgmentColors["DD"][5], -- FA+ mode doesn't have a Way Off window. Extract color from the DD mode.
	...
	TapNoteScores.Names = {
		...
		THEME:GetString("TapNoteScore", "W5"), -- FA+ mode doesn't have a Way Off window. Extract name from the DD mode.
	...
	local tns_string = "TapNoteScore" .. (SL.Global.GameMode=="DD" and "" or SL.Global.GameMode)
...
for i=1, #TapNoteScores.Types do
	-- no need to add BitmapText actors for TimingWindows that were turned off
		if windows[i] or i == #TapNoteScores.Types or (mods.ShowFaPlusWindow and windows[i-1]) then
